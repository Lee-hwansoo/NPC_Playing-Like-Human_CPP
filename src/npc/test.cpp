...
void testIntegratedObjects(SDL_Renderer* renderer) {
    ...
    // 에이전트 생성 및 초기화
    auto agent = std::make_unique<object::Agent>(std::nullopt, std::nullopt, constants::Agent::RADIUS, constants::Agent::SPAWN_BOUNDS, constants::Agent::MOVE_BOUNDS, Display::to_sdl_color(Display::BLUE), true, circle_obstacles_state, rectangle_obstacles_state, goal->get_state());
    ...
    while (!quit) {
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT ||
                ...
                agent->reset(std::nullopt, std::nullopt, circle_obstacles_state, rectangle_obstacles_state, goal->get_state());
            }
        }
